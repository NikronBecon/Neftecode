...
class HierMPNDecoder(nn.Module):
    ...
    def __init__(self, vocab, avocab, rnn_type, embed_size, hidden_size, latent_size, depthT, depthG, dropout, attention=False):
        ...
        self.E_order = torch.eye(MolGraph.MAX_POS).to("cpu")
    ...
    def get_assm_score(self, src_graph_vecs, batch_idx, assm_vecs):
        if assm_vecs.size(0) == 0:
            print("Empty assm_vecs. Returning empty tensor.")
            return torch.empty(0).to(src_graph_vecs.device)
        if self.use_attention:
    ...
    def enum_attach(self, hgraph, cands, icls, nth_child):
        if len(cands) == 0 or len(icls) == 0:
            print("Empty cands or icls. Returning empty tensor.")
            return torch.empty(0, self.hidden_size).to(self.device)
        ...
        if cand_vecs.size(0) == 0 or icls_vecs.size(0) == 0 or order_vecs.size(0) == 0:
            print("One of the tensors is empty. Skipping concatenation.")
            return torch.empty(0, self.hidden_size).to(self.device)  # Return an empty tensor
                ...
                for kk in cls_beam: #try until one is chemically valid
                    ...
                    if len(inter_cands) == 0:
                        ...
                        for x in anchor_smiles:
                            key = (smiles, x)
                            try:
                                icls.append(self.vocab[key][1])
                            except KeyError:
                                print(f"Warning: Key not found in vocabulary: {key}")
