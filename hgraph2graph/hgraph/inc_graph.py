...
class IncBase:
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes, max_edges, max_nb):
        ...
        self.fnode = torch.zeros(max_nodes * batch_size, node_fdim).long().to(self.device)
        ...
        self.max_nb = max_nb  # Ensure max_nb is properly initialized
...
class IncTree(IncBase):
    def __init__(self, batch_size, node_fdim, edge_fdim, max_nodes=100, max_edges=200, max_nb=12, max_sub_nodes=20):
        ...
        self.cgraph = self.fnode.new_zeros(max_nodes * batch_size, max_sub_nodes).to(self.device)
...
class IncGraph(IncBase):
    ...
    def get_atom_feature(self, atom):
        f = torch.zeros(self.avocab.size()).long()
        symbol, charge = atom.GetSymbol(), atom.GetFormalCharge()
        f[self.avocab[(symbol, charge)]] = 1
        return f.to(self.device)  # Dynamically allocate to the correct device
    ...
    def get_mess_feature(self, atom, bond_type, nth_child):
        ...
        f3[ nth_child ] = 1
        return torch.cat( [f1,f2,f3], dim=-1 ).to(self.device)
