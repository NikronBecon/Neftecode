...
class MolGraph(object):
    ...
    def __init__(self, smiles):
        try:
            ...
            raise ValueError(f"Failed to process molecule: {smiles} — {e}")
    ...
    def tensorize(mol_batch, vocab, avocab):
        ...
        tree_tensors, tree_batchG = MolGraph.tensorize_graph([x.mol_tree for x in valid_mols], vocab)
        graph_tensors, graph_batchG = MolGraph.tensorize_graph([x.mol_graph for x in valid_mols], avocab)
        tree_scope = tree_tensors[-1]
        graph_scope = graph_tensors[-1]

        max_cls_size = max([len(c) for x in valid_mols for c in x.clusters])
        ...
        all_orders = []
        for i, hmol in enumerate(valid_mols):
