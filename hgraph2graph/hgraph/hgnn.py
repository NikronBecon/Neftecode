...
class HierVAE(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = HierMPNEncoder(
            ...
            args.hidden_size, args.depthT, args.depthG, args.dropout, device=self.device
        )
        self.decoder = HierMPNDecoder(  # Remove `device` argument here
            args.vocab, args.atom_vocab, args.rnn_type, args.embed_size,
        ...
        self.R_mean = nn.Linear(args.hidden_size, args.latent_size).to(self.device)
        self.R_var = nn.Linear(args.hidden_size, args.latent_size).to(self.device)

    # Other methods remain unchanged
    def rsample(self, z_vecs, W_mean, W_var, perturb=True):
        device = z_vecs.device  # Получаем device из входного тензора
        ...
        epsilon = torch.randn_like(z_mean, device=device)
        z_vecs = z_mean + torch.exp(z_log_var / 2) * epsilon if perturb else z_mean
        return z_vecs, kl_loss


    def sample(self, batch_size, greedy):
        device = self.R_mean.weight.device
...
class HierVGNN(nn.Module):
    ...
    def translate(self, tensors, num_decode, enum_root, greedy=True):
        ...
        z_tree = torch.randn(batch_size, 1, self.latent_size).expand(-1, tree_vecs.size(1), -1).to(tree_vecs.device)
        z_graph = torch.randn(batch_size, 1, self.latent_size).expand(-1, graph_vecs.size(1), -1).to(graph_vecs.device)
...
class HierCondVGNN(HierVGNN):
    ...
    def translate(self, tensors, cond, num_decode, enum_root):
        ...
        z_tree = torch.randn(num_decode, 1, self.latent_size).expand(-1, tree_vecs.size(1), -1).to(tree_vecs.device)
        z_graph = torch.randn(num_decode, 1, self.latent_size).expand(-1, graph_vecs.size(1), -1).to(graph_vecs.device)
    ...
    def forward(self, x_graphs, x_tensors, y_graphs, y_tensors, y_orders, cond, beta):
        x_tensors = make_cuda(x_tensors)
        y_tensors = make_cuda(y_tensors)
        cond = torch.tensor(cond).float().to(x_tensors[0].device)
